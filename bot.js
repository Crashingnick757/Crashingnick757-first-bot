...
client.user.setPresence({ game: { name: 'prefix is: c!' }, status: 'online' })
  .then(console.log)
  .catch(console.error);
});
...
if (command === "test_confirm" || command === "testconfirm"){
message.channel.send("OK").catch((e) => { console.log(e); });
console.log('Test confirmed');
}

if (command === "ping_me" || command === "pingme"){
...
if (message.author.id !== '415955266853535759') {
message.channel.send('This is a command for DEVS ONLY!');
}
if (message.author.id == '415955266853535759') {
...
console.log('DMed '+user.tag);//asdfg
...
user.send('If applying for moderator in Crashingnick757 please use this link:').catch((e) => { console.log(e); });
}
*/
...
if (command === "apply"){
const user = message.author;
user.send('Sorry, but mod applications are not currently being accepted.').catch((e) => { console.log(e); });
}
...
if (command === "devs" || command === "devlist" || command === "dev_list"){
...
if (command === 'roll_a_die' || command === 'dice_roll'){
...
var num_ = getRandomInt(1,max_number);
message.channel.send(num_);
